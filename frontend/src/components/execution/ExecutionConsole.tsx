...
export function ExecutionConsole({ error, onClear, onRerun, result, submitting }: ExecutionConsoleProps) {
  ...
  const statusClass = result?.status.toLowerCase() ?? (submitting ? 'running' : 'idle');
  const statusLabel = result ? executionStatusLabel(result.status) : submitting ? 'Submitting' : 'Ready';

  useLayoutEffect(() => {
    const output = outputRef.current;
    if (output && nearBottomRef.current) {
      output.scrollTop = output.scrollHeight;
    }
  }, [error, result, submitting]);

  return (
    <section className="execution-console" aria-label="Execution console" id="execution-console">
      <header className="execution-console-header">
        <div className="execution-console-title">
          <SquareTerminal size={14} />
          <strong>Console</strong>
          <span className={`execution-status execution-status-${statusClass}`}>{statusLabel}</span>
      ...
      <div
        ...
        aria-busy={running}
        aria-live="polite"
        onScroll={(event) => { nearBottomRef.current = isNearBottom(event.currentTarget); }}
        ref={outputRef}
      >
